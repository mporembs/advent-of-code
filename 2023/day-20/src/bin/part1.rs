...
fn part1(input: &'static str) -> String {
    ...
    let mut pulses_per_cycle: Vec<(u64, u64)> = Vec::new();

    for _ in 0..1000 {
        pulses_per_cycle.push(cycle(&mut modules, &init_pulses));
    }
    let tuple_sum = pulses_per_cycle
        .iter()
        .fold((0, 0), |(ach, acl), (h, l)| (ach + h, acl + l));
    dbg!(tuple_sum);

    (tuple_sum.0 * tuple_sum.1).to_string()
}

fn cycle(modules: &mut HashMap<&str, Module>, init_pulses: &Vec<Pulse>) -> (u64, u64) {
    let mut pulse_counter: (u64, u64) = (0, init_pulses.len() as u64 + 1);
            ...
            match active_module {
                Module::FlipFlop(active_ff) => {
                    ...
                    if let Some(new_pulses) = get_pulse_result {
                        new_pulses.iter().for_each(|new_pulse| {
                            match new_pulse.signal {
                                PulseType::High => pulse_counter.0 += 1,
                                PulseType::Low => pulse_counter.1 += 1,
                            }
