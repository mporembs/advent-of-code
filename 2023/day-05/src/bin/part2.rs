...
fn part2(input: &str) -> String {
    ...
    let mut seeds_numbers = section_iter
        .next()
        .expect("a string")
        .split(' ')
    ...
    for _number in 0..seeds_numbers.clone().count() / 2 {
        let start = seeds_numbers.next().expect("a number");
        seed_ranges.push(Range {
            start: start,
            end: start + seeds_numbers.next().expect("a number"),
        })
    }
    ...
    let _location_func: Vec<_> = seed_ranges
        .iter()
        .enumerate()
        .map(|thing| {
            for seed_number in thing.1.start..thing.1.end {
                let soil_number: u64 = {
                ...
                let water_number: u64 = {
                    let converted = converters
                        ...
                        .map(|converter| {
                            match converter.source_range.contains(&fertilizer_number) {
                                true => {
                                    let source_index =
                ...
                let humidity_number: u64 = {
                    let converted = converters
                        ...
                        .map(|converter| {
                            match converter.source_range.contains(&temperature_number) {
                                true => {
                                    let source_index =
                    ...
                    let converted = converters
                        ...
                        .map(
                            |converter| match converter.source_range.contains(&humidity_number) {
                                true => {
                                    let source_index =
                                        &humidity_number - converter.source_range.start;
                                    converter.dest_range.start + source_index
                                }
                                false => 0,
                            },
                ...
                // println!("Seed #{seed_number}: yields location {location_number}");
                locations.push(location_number);
            }
        })
        .collect();
