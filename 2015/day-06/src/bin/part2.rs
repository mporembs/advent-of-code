...
fn part2(input: &str) -> String {
    let mut light_status: HashMap<IVec2, u128> = HashMap::new();

    (0..1_000).for_each(|row| {
        (0..1_000).for_each(|col| {
            light_status.insert(IVec2::new(col, row), 0);
        })
    });
            ...
            instuction.col_range.clone().for_each(|col_idx| {
                match instuction.command {
                    LightCommand::On => light_status
                        ...
                        .and_modify(|status| *status += 1),
                    LightCommand::Off => light_status
                        .entry(IVec2::new(col_idx as i32, row_idx as i32))
                        .and_modify(|status| {
                            match status.checked_sub(1) {
                                Some(_) => *status -= 1,
                                None => *status = 0,
                            };
                        }),
    ...
    let on: u128 = light_status.into_values().sum();
    on.to_string()
}
...
toggle 0,0 through 999,999",
        ...
        assert_eq!(result, "2000001".to_string());
    }
}
